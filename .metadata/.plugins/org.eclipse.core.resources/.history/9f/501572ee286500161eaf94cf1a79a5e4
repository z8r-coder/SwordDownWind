...
public class HeroInfo {
	...
	public static Vector<JLabel> initInfo(){
		...
		head_photo.setBounds(fightHead_x + 10, fightHead_y, fightHead_image_width, fightHead_image_height);
		...
		HeroInfo.add(attack);
		HeroInfo.add(ex);
		HeroInfo.add(head_photo);
		return HeroInfo;
	}
	public static void updateInfo(Vector<JLabel> HeroInfo){
		HeroInfo.get(0).setText("姓名：" + player.getname());
		HeroInfo.get(1).setText("等级：" + player.getLevel());
		HeroInfo.get(2).setText("HP：" + player.getnowHP() + "//" + player.getlimitedHP());
		HeroInfo.get(3).setText("MP：" + player.getnowMP() + "//" + player.getlimitedMP());
		HeroInfo.get(4).setText("攻击力：" + player.getBaseAttack());
		HeroInfo.get(5).setText("经验：" + player.getEX() + " / " + player.getUpEx());
	}
}
