...
public class GetInfoOfMom extends JFrame implements ActionListener{
	...
	public GetInfoOfMom(Hero player) {
		...
		MyDialog.showMessageDialog("                  万里飘香便在清州靠近河岸处！", "提示", LenthAll.TALK_DIALOG_WIDTH, LenthAll.TALK_DIALOG_HEIGHT);
		...
		this.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
					...
					Thread td_Goods = new Thread(new Runnable() {
						...
						public void run() {
							...
							if(Math.random() > Double.parseDouble(ms.getMonsterInfo(randNum)[9]) && randNum == 0){
								MyDialog.showMessageDialog("                 恭喜您，获得一棵闭月花！", "提示", LenthAll.TALK_DIALOG_WIDTH, LenthAll.TALK_DIALOG_HEIGHT);
								player.addMedicament("闭月花", new biyuehua(), 1);
							}
							else if(Math.random() > Double.parseDouble(ms.getMonsterInfo(randNum)[9]) && randNum == 1){
								MyDialog.showMessageDialog("                 恭喜您，获得一颗长离草！", "提示", LenthAll.TALK_DIALOG_WIDTH, LenthAll.TALK_DIALOG_HEIGHT);
								player.addMedicament("长离草", new Changlicao(), 1);
							}
						}
					});
					td_Goods.start();
				...
				if(Person_x == 16&&Person_y == 4){
					MyDialog.showMessageDialog("            十载岁月，终于可以离真相更近一步了！", "燕惊寒", LenthAll.TALK_DIALOG_WIDTH, LenthAll.TALK_DIALOG_HEIGHT);
					new jiulou();
					setVisible(false);
				}
			}
		...
		exit.setFocusable(false);//消除读档后button获取焦点问题
	}
